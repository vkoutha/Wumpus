...
public class Tile {
	...
	public static void updateTilesOpacity() {
		for (Tile[] tileArr : Game.game.getTiles()) {
			for (Tile tile : tileArr) {
				if (tile.isFlashlightAffected() && tile.getOpacity() <= GameData.FLASHLIGHT_BLOCK_OPACITY.getAlpha()
						- GameData.TILE_DIMMING_SPEED) {
					tile.setOpacity(tile.getOpacity() + GameData.TILE_DIMMING_SPEED);
				} else if (tile.isFlashlightAffected()
						&& tile.getOpacity() >= GameData.FLASHLIGHT_BLOCK_OPACITY.getAlpha()
								+ GameData.TILE_BRIGHTENING_SPEED) {
	...
	private boolean isFlashlightAffectedCalc() {
		...
		for (int r = Player.getRow() - Toolbar.getFlashlightRadius(); r <= Player.getRow()
				+ Toolbar.getFlashlightRadius(); r++) {
			for (int c = Player.getColumn() - Toolbar.getFlashlightRadius(); c <= Player.getColumn()
					+ Toolbar.getFlashlightRadius(); c++) {
				if (row == r && column == c) {
					return true;
				}
...
//				}
	...
	public void render(Graphics g) {
		...
		if (isDiscovered || isFlashlightAffected || opacity <= .99f) {
			g2.drawImage(GameData.grassSprite, (column * GameData.TILE_WIDTH), (row * GameData.TILE_HEIGHT),
					GameData.TILE_WIDTH + 1, GameData.TILE_HEIGHT, null);
			if (item != ItemTypes.NONE) {
				g2.drawImage(itemSprite, column * GameData.TILE_WIDTH, row * GameData.TILE_HEIGHT, GameData.TILE_WIDTH,
		...
		if (!isDiscovered)
			g2.drawImage(GameData.rockSprite, (column * GameData.TILE_WIDTH), (row * GameData.TILE_HEIGHT),
					GameData.TILE_WIDTH, GameData.TILE_HEIGHT, null);
	}
